        ...
        <div className={`panel-container relative group ${isFullBleed ? '!p-0 !bg-[#050505]' : ''}`}>
            ...
            {face.isDecisionPage && face.choices.length > 0 && (
                <div className={`absolute bottom-0 inset-x-0 p-6 pb-12 flex flex-col gap-3 items-center justify-end transition-opacity duration-500 ${face.resolvedChoice ? 'opacity-0 pointer-events-none' : 'opacity-100'} bg-gradient-to-t from-black via-black/80 to-transparent z-20`}>
                    <p className="text-[#39FF14] font-mono text-xl uppercase tracking-widest animate-pulse bg-black px-2">Choose The Vibe</p>
                    {face.choices.map((choice, i) => (
                        <button key={i} onClick={(e) => { e.stopPropagation(); if(face.pageIndex) onChoice(face.pageIndex, choice); }}
                          className={`comic-btn w-full py-4 text-xl font-bold tracking-wider uppercase border-2 ${i===0?'bg-[#39FF14] text-black border-black hover:bg-white':'bg-black text-white border-[#39FF14] hover:bg-gray-900'}`}>
                            {choice}
                        </button>
                    ))}
                </div>
            ...
            {face.type === 'cover' && (
                 <div className="absolute bottom-20 inset-x-0 flex justify-center z-20">
                     <button onClick={(e) => { e.stopPropagation(); onOpenBook(); }}
                      ...
                      className="comic-btn bg-[#39FF14] text-black border-black px-10 py-4 text-3xl font-bold hover:scale-105 disabled:bg-gray-600 disabled:text-gray-400 disabled:border-gray-600">
            ...
            {face.type === 'back_cover' && (
                <div className="absolute bottom-24 inset-x-0 flex flex-col items-center gap-4 z-20">
                    <button onClick={(e) => { e.stopPropagation(); onDownload(); }} className="comic-btn bg-white text-black border-black px-8 py-3 text-xl font-bold hover:scale-105 uppercase">SAVE TO CAMERA ROLL</button>
                    <button onClick={(e) => { e.stopPropagation(); onReset(); }} className="comic-btn bg-[#39FF14] text-black border-black px-8 py-4 text-2xl font-bold hover:scale-105 uppercase animate-bounce">DROP NEXT EPISODE 💀</button>
                </div>
            )}
        </div>
    );
